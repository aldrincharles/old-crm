...
const OrganizationStrategyAdd = ({ onAdd }) => {
...
const ComponentDialogue = React.memo(({ onAdd, visible, toggle }) => {
  ...
  const onSubmit = async (event) => {
    ...
    const response = authAxios.post(`organization/${id}/strategy`, {
      info,
      criteria,
    });
    ...
    setInfo({ ...info, summary: "", remarks: "" });
    setCriteria([{ inputType: "" }]);
    goTo(0);
    toggle();
    onAdd();
  };
  ...
  return (
    <>
      <BootstrapModal size="lg" isOpen={visible} toggle={toggle}>
        <BootstrapModal.Header>Add Strategy</BootstrapModal.Header>
        <BootstrapModal.Body>
...
export { OrganizationStrategyAdd };
