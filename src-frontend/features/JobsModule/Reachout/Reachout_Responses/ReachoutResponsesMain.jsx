...
import { ReachoutResponsesDisplay } from "./ReachoutResponsesDisplay";
import { toast } from "react-toastify";

export const ReachoutResponsesMain = () => {
  const authAxios = useAxiosPrivate();
  const { id } = useParams();
  const [selectedRow, setSelectedRow] = useState([]);
  const { data, setData, isLoading, errorMessage, refetch } = useFetch({
    initialUrl: `/jobs/reachout/responses/${id}`,
  });
  ...
  const handleDelete = async () => {
    let listIDs = [];
    let tableStage = 2; //I put 2 here because Responses are in stage 2
