...
import { ReachoutClientInterviewDisplay } from "./ReachoutClientInterviewDisplay";
import { toast } from "react-toastify";

export const ReachoutClientInterviewMain = () => {
  const authAxios = useAxiosPrivate();
  const { id } = useParams();
  const [selectedRow, setSelectedRow] = useState([]);

  const { data, setData, isLoading, errorMessage, refetch } = useFetch({
    initialUrl: `/jobs/reachout/client-interview/${id}`,
  });
  ...
  const handleDelete = async () => {
    let listIDs = [];
    let tableStage = 5; //stage 5- client interview
  ...
  return (
    <>
      ...
      {data ? (
        <ReachoutClientInterviewDisplay
          data={data}
