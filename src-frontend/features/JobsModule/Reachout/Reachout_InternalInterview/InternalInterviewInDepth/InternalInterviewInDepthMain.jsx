...
import { InternalInterviewInDepthDisplay } from "./InternalInterviewInDepthDisplay";
export const InternalInterviewInDepthMain = () => {
  const authAxios = useAxiosPrivate();
  const { id } = useParams();
  const [selectedRow, setSelectedRow] = useState([]);

  const { data, setData, isLoading, errorMessage, refetch } = useFetch({
    initialUrl: `/jobs/reachout/internal-interview/in-depth/${id}`,
  });
  ...
  return (
    <>
      ...
      {data ? (
        <InternalInterviewInDepthDisplay
          data={data}
