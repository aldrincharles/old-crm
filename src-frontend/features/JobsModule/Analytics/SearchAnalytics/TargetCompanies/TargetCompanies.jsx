...
const TargetCompanies = ({ id }) => {
  ...
  const { data, isLoading, errorMessage, refetch, updateParams } = useFetch({
    initialUrl: `/analytics-report/sourcing-movement/target-companies/${id}`,
  ...
  return (
    <>
      ...
      >
        <span style={{ fontSize: "125px" }}>{data?.total || ""}</span>
      </Card>
    </>
  );
};

export { TargetCompanies };
